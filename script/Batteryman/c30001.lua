...
function s.tstg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return e:GetHandler():CanSummonOrSet(true,nil,1) end
	Duel.SetOperationInfo(0,CATEGORY_SUMMON,e:GetHandler(),1,tp,LOCATION_HAND)
end
function s.tsop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if c:IsRelateToEffect(e) then Duel.Summon(tp,c,true,nil,1) end
end
...
function s.valcheck(e,c)
	...
	while tc do
		...
		elseif code==63142001 then flag=(flag|0x4) end
		tc=g:GetNext()
	end
	e:SetLabel(flag)
end
...
function s.rmvcon(e,tp,eg,ep,ev,re,r,rp)
	return not e:GetHandler():IsStatus(STATUS_BATTLE_DESTROYED) and re:IsTrapEffect() and re:IsHasType(EFFECT_TYPE_ACTIVATE) and Duel.IsChainNegatable(ev)
end
...
function s.drcon(e,tp,eg,ep,ev,re,r,rp)
	...
	return flag and (flag&0x4)~=0 and eg:IsExists(Card.IsType,1,nil,TYPE_MONSTER)
end
