...
-- Logic Triệu hồi: Kiểm tra có Thunder trên sân để active effect từ tay
...
-- Kiểm tra nguyên liệu (Bitwise Flag)
function s.valcheck(e,c)
    local g=c:GetMaterial()
    local flag=0
    for tc in g:Iter() do
        local code=tc:GetCode()
        if code==55401221 then flag=(flag|0x1) end
        if code==19733961 then flag=(flag|0x2) end
        if code==63142001 then flag=(flag|0x4) end
        if code==47346845 then flag=(flag|0x8) end
    end
    e:SetLabel(flag)
end
...
function s.regop(e,tp,eg,ep,ev,re,r,rp)
    ...
    -- Effect 1: Negate Spell (Thêm Once Per Turn)
    if (flag&0x1)~=0 then
        local e1=Effect.CreateEffect(c)
        ...
        e1:SetCountLimit(1) -- Tránh spam vô hạn
    ...
    -- Effect 2: Banish Trap (Thêm Once Per Turn)
    if (flag&0x2)~=0 then
        local e2=Effect.CreateEffect(c)
    ...
    -- Effect 3: Draw 2 (Nếu muốn Draw ngay khi Summon chính mình)
    if (flag&0x4)~=0 then
        Duel.Hint(HINT_CARD,0,id)
        Duel.Draw(tp,2,REASON_EFFECT)
        
        -- Nếu vẫn muốn Draw khi các quái khác summon sau đó:
